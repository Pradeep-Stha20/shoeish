...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
        ...
        if(isset($_POST['ssd'])) {
        	...
        	$statement = $pdo->prepare("DELETE FROM tbl_product_ssd WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['ssd'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_ssd (ssd_id,p_id) VALUES (?,?)");
		...
		if(isset($_POST['ram'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_product_ram WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['ram'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_ram (ram_id,p_id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_product_ram WHERE p_id=?");
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select ssd</label>
							<div class="col-sm-4">
								<select name="ssd[]" class="form-control select2" multiple="multiple">
									<?php
									$is_select = '';
									$statement = $pdo->prepare("SELECT * FROM tbl_ssd ORDER BY ssd_id ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);			
									foreach ($result as $row) {
										if(isset($ssd_id)) {
											if(in_array($row['ssd_id'],$ssd_id)) {
												$is_select = 'selected';
											} else {
												$is_select = '';
											}
										...
										?>
										<option value="<?php echo $row['ssd_id']; ?>" <?php echo $is_select; ?>><?php echo $row['ssd_capacity']; ?></option>
										<?php
									}
									?>
								</select>
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select RAM</label>
							<div class="col-sm-4">
								<select name="ram[]" class="form-control select2" multiple="multiple">
									<?php
									$is_select = '';
									$statement = $pdo->prepare("SELECT * FROM tbl_ram ORDER BY ram_id ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);			
									foreach ($result as $row) {
										if(isset($ram_id)) {
											if(in_array($row['ram_id'],$ram_id)) {
												$is_select = 'selected';
											} else {
												$is_select = '';
											}
										...
										?>
										<option value="<?php echo $row['ram_id']; ?>" <?php echo $is_select; ?>><?php echo $row['ram_capacity']; ?></option>
										<?php
									}
									?>
								</select>
