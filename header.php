			...
			<div class="col-md-5 right">
				<ul>
					...
					if(isset($_SESSION['customer'])) {
						...
						<li><a href="logout.php"><i class="fa fa-sign-out"></i> <?php echo LANG_VALUE_14; ?></a></li>
						<?php
					} else {
						?>
				...
				<div class="menu-container">
					<div class="menu">
						<ul>
							...
							<?php
							$statement = $pdo->prepare("SELECT * FROM tbl_top_category WHERE show_on_menu=1 LIMIT 5");
							$statement->execute();
							$result = $statement->fetchAll(PDO::FETCH_ASSOC);
							foreach ($result as $row) {
								?>
								<li><a href="product-category.php?id=<?php echo $row['tcat_id']; ?>&type=top-category"><?php echo $row['tcat_name']; ?></a>
									<ul>
										<?php
										$statement1 = $pdo->prepare("SELECT * FROM tbl_mid_category WHERE tcat_id=? LIMIT 5" );
										...
										foreach ($result1 as $row1) {
											...
											<li><a href="product-category.php?id=<?php echo $row1['mcat_id']; ?>&type=mid-category"><?php echo $row1['mcat_name']; ?></a>
												<ul>
													<?php
													$statement2 = $pdo->prepare("SELECT * FROM tbl_end_category WHERE mcat_id=? LIMIT 5" );
